...
namespace MathsProblems
{
    ...
    {
        private const int xMax = 14;
        private static readonly int[,]  piramid = new int[,]{
        ...
        internal static string Maximum_path_sum_I()
        {
            var sum = 0;
            sum = Max(0, 0);
            return sum.ToString();
        ...
        internal static int Max(int x, int y)
        {
            ...
            if (x == xMax)
                return piramid[x,y];
            maxLeft = Max(x + 1, y);
            maxRight = Max(x + 1, y + 1);
            sum = piramid[x,y] + Math.Max(maxRight, maxLeft);
            return sum;
